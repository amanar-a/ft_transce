...
export class AuthController {
  ...
  async asyncgoogleAuthRedirect(
    ...
    try {
      ...
      if (ret == 1)
        response.redirect(
          `http://10.13.3.3:3000/authentication?token=${info.refAcc.accessToken}&refreshToken=${info.refAcc.refreshToken}`,
        );
      else if (ret == 2)
        response.redirect(
          `http://10.13.3.3:3000/home?token=${info.refAcc.accessToken}&refreshToken=${info.refAcc.refreshToken}`,
        );
      else response.redirect(`http://10.13.3.3:3000`);
      // } else {
      //   if (ret == 1)
      //   response.redirect(
