...
const refresh_token_dto_1 = require("../dto-classes/refresh-token.dto");
const ft42_guard_1 = require("../guards/ft42.guard");
...
let AuthController = class AuthController {
    ...
    async asyncgoogleAuthRedirect(req, response, ip) {
      ...
      FROM public."Users" WHERE "email" = '${info.email}'; `);
            ...
            if (ret == 1)
                response.redirect(`http://10.13.1.13:3000/authentication?token=${info.refAcc.accessToken}&refreshToken=${info.refAcc.refreshToken}`);
            else if (ret == 2)
                response.redirect(`http://10.13.1.13:3000/home?token=${info.refAcc.accessToken}&refreshToken=${info.refAcc.refreshToken}`);
            else
                response.redirect(`http://10.13.1.13:3000`);
        }
        catch (e) {
            console.log(e);
        }
...
__decorate([
    (0, common_1.Get)(),
    (0, common_1.UseGuards)(ft42_guard_1.Ft42AuthGuard),
